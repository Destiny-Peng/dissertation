...
| `--instruction-condition CONDITION` | `full_instruction` | all | Selects `full_instruction`, `subtask_a` (A), or `subtask_b` (B) from a combined instruction-variant manifest. Full instruction keeps source rollout IDs; A/B use `<source-id>--subtask_a|subtask_b` and should be written to an isolated output parent. |
| `--data-root PATH`    | project root            | all                               | Root used for relative `camera_video_paths` and `csv_path` values. Source media are read in place and are not copied or modified.                                                              |
...
The wrapper saves sampled indices, the absolute value-head outputs, analysis text, parsed analysis, and model/video metadata in `raw_model_outputs.json`. RynnValue raw output additionally records `sampling_mode=approximate_fixed_interval_prefix_uniform`, the requested `evaluation_interval`, and the computed `num_steps`; existing `values` and `sampled_indices` arrays remain unchanged for temporal analysis. The same official model forward also returns the signed `<relative_value>` head. New raw files preserve `relative_values_by_prefix` (every native relative slot), `relative_values` (the last relative slot for each prefix, aligned to `sampled_indices` for Review), `relative_sampled_indices`, and `relative_slot_counts`. The wrapper does not run a second inference or modify the upstream RynnValue repository. Older raw files remain valid and simply have no relative signal until rerun.
