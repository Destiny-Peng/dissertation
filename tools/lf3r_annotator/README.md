...
The snapshot API returns method coverage, method/signal/outcome summaries, onset threshold/direction statistics, compact temporal event metrics, rollout-level terminal outcome classification, and the failure-localization summary/threshold/failure-type tables joined with rollout and task metadata. Terminal outcome classification treats terminal failure as positive, clean/recovered success as negative, excludes uncertain rollouts from metric denominators, and reports Q90/Q95/Q99 success-calibrated thresholds with Accuracy, failure Recall, Precision, F1, Specificity/FPR, Balanced Accuracy, and AUROC. Large frame arrays are intentionally removed before JSON is sent to the browser. The page visualizes coverage, recovered-versus-terminal normalized response magnitude (median/IQR), post-event persistence, recovered-event recovery-to-baseline fraction, clean-success Q95 threshold exceedance/direction consistency, and the top anomalous event table. Method, snapshot-outcome, and snapshot-task filters are independent of the live filters; Review buttons navigate back to the corresponding rollout.
...
- server.py serves static UI files, health and rollout APIs, video byte ranges, annotation read/write endpoints, shared Settings APIs, the read-only Analysis API, baseline evaluation/job APIs, batch baseline jobs, temporal-analysis jobs, and rollout-generation jobs. Manifest camera video paths, output paths, and job logs are confined to PROJECT_ROOT.
...
- The per-rollout **Run baseline** button follows the selected condition. A/B jobs receive the variant rollout ID (`<source-id>--subtask_a` or `--subtask_b`), use the variant manifest, and write under `outputs/baselines/instruction_variants/libero_10/<condition>/`. Full-instruction raw directories are never reused for an A/B view.
- The Batch baseline panel has its own **Instruction condition** selector. It filters the available rollout count and worker index space to the selected condition; selecting A or B therefore runs only prepared variants, while Full instruction keeps the source manifest and existing outputs.
- Tasks without a validated A/B variant remain view-only for that condition. The selector never changes the annotation key, video, or rollout queue.
...
The **Existing result filter** can keep all matching rollouts or select only **No valid baseline run result**. The latter is method- and instruction-condition-aware and is also annotation-aware: only source rollouts with `review_status=complete` are eligible; `in_progress` and `unreviewed` are excluded before worker ranges are built. Among complete annotations, the server checks existing outputs with the same parser used by Review, skips only rollouts with a parseable valid result, and keeps failed, missing, empty, or unreadable baseline outputs eligible. The filtered rollout list becomes the authoritative scope-relative index space for worker `[start,end)` ranges. The browser refreshes this coverage before submission, and the server recomputes both annotation status and baseline coverage again when accepting the batch so stale UI state cannot silently rerun or include unfinished annotations.
...
After the run note, both wrappers accept optional flags:

- `--render-resolution N` sets the square simulator render width/height;
- `--record-resolution N` sets the square saved MP4 width/height;
- `--log-safe-features` enables the existing SAFE latent sidecars.

Resolution values must be even integers from 64 through 2048. The defaults are render `256x256`, record `224x224` for LIBERO-10, and record `256x256` for LIBERO-Spatial. These dimensions are independent: the OpenVLA checkpoint still receives its official `224x224` policy input after preprocessing. For example:

    bash tools/lf3r_annotator/generate_libero10_natural.sh 0 0 0 1 7 lf3r-data-natural-libero10-custom --render-resolution 384 --record-resolution 320
...
**Runs → Utilities → Batch H.264 transcode** shows every manifest currently loaded by the WebUI as a selectable checkbox (all selected by default, with Select all/Clear controls). Every video declared in `camera_video_paths` for the selected manifests is processed. Paths are deduplicated across cameras and manifests; already-H.264 files are skipped, and each conversion reuses `transcode_video_h264.sh`: the original becomes `.orig.mp4` and the H.264/yuv420p replacement keeps the original manifest path. Existing `.orig.mp4` backups are treated as conflicts and are never overwritten. Single-video and batch transcode jobs are mutually exclusive to prevent two ffmpeg jobs from racing the same file.
