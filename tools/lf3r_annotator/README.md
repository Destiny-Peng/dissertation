...
Rollout generation supports the default `single_view` recording and `libero_three_view`. The three-view mode preserves the canonical rollout and replays the recorded actions without model inference to write three independent MP4 files matching Robo-Dopamine's fixed input slots: `<stem>.cam_high.mp4`, `<stem>.cam_left_wrist.mp4`, and `<stem>.cam_right_wrist.mp4`. For LIBERO the source-camera adapter is `cam_high <- agentview`, `cam_left_wrist <- robot0_eye_in_hand`, and `cam_right_wrist <- sideview`; the manifest records both `camera_video_paths` and `camera_source_names` so the simulator source is explicit. All three camera videos must have the same frame count and FPS as the canonical rollout.

The manifest no longer contains `multiview_video_path`, `multiview_layout`, or `multiview_cameras`. Robo-Dopamine reads the three official keys from `camera_video_paths`; when no official three-view mapping is present it follows the upstream single-view pattern and repeats canonical `video_path` for all three inputs. The existing Review player remains canonical by default; a specific camera file can be served explicitly with `/api/videos/<rollout-id>?camera=<slot>`.
