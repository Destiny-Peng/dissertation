...
**Runs → Utilities → Batch H.264 transcode** shows every manifest currently loaded by the WebUI as a selectable checkbox (all selected by default, with Select all/Clear controls). Only the canonical `video_path` entries from the selected manifests are processed; `camera_video_paths` and legacy `multiview_video_path` fields are ignored. Paths are deduplicated, already-H.264 files are skipped, and each conversion reuses `transcode_video_h264.sh`: the original becomes `.orig.mp4` and the H.264/yuv420p replacement keeps the original manifest path. Existing `.orig.mp4` backups are treated as conflicts and are never overwritten. Single-video and batch transcode jobs are mutually exclusive to prevent two ffmpeg jobs from racing the same file.
...
### Multiview camera videos

Rollout generation supports the default `single_view` recording and `libero_three_view`. The three-view mode preserves the canonical single-view rollout and policy input, then replays the recorded actions without model inference and writes three independent MP4 files beside the canonical rollout: `<stem>.agentview.mp4`, `<stem>.sideview.mp4`, and `<stem>.robot0_eye_in_hand.mp4`. The manifest keeps `video_path` as the canonical baseline input and records the separate camera files in `camera_video_paths`; `multiview_video_path` is not used for this representation. All three camera videos must have the same frame count and FPS as the canonical rollout. The existing single Review player continues to show the canonical `video_path`; the separate camera paths are available for future multiview model input or a dedicated synchronized-view UI.
