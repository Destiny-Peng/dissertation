...
The **Existing result filter** can keep all matching rollouts or select only **No valid baseline run result**. The latter is method- and instruction-condition-aware: before launch the server checks existing completed outputs with the same parser used by Review, skips only rollouts with a parseable valid result, and keeps failed, missing, empty, or unreadable outputs eligible. The filtered rollout list becomes the authoritative scope-relative index space for worker `[start,end)` ranges. The browser refreshes this coverage before submission, and the server recomputes it again when accepting the batch so newly completed results are not silently rerun.
