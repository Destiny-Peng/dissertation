...
class ProgressiveBaselineResultsContractTest(unittest.TestCase):
    def test_server_exposes_only_completed_rollouts_from_running_runs(self) -> None:
        ...
        baseline_results = (TOOL_ROOT / "baseline_results.py").read_text(encoding="utf-8")
        self.assertIn('workers_root.glob("worker-*/jobs.jsonl")', core_source)
        self.assertIn('row.get("status") != "complete"', core_source)
        self.assertIn("def _valid_result_rollout_ids(", baseline_results)
        self.assertIn("for run_path, metadata in self._run_candidates(baseline):", baseline_results)
        self.assertIn("completed_ids = run_rollout_ids(run_path)", baseline_results)
        self.assertIn("self._read_method(", baseline_results)
