...
class DatasetAndFrontendContractTest(unittest.TestCase):
    ...
    def test_openvla_safe_feature_wrapper_contract(self) -> None:
        ...
        self.assertIn('"cam_high": "agentview"', multiview)
        self.assertIn('"cam_left_wrist": "robot0_eye_in_hand"', multiview)
        self.assertIn('"cam_right_wrist": "sideview"', multiview)
        self.assertIn("ACTION_FIELDS", multiview)
        self.assertIn('"camera_video_paths"', multiview)
        self.assertIn('"camera_source_names"', multiview)
        self.assertIn('video.stem + f".{slot}.mp4"', multiview)
