...
class NonAnalysisToolTests(unittest.TestCase):
    ...
    def test_runs_ui_exposes_external_rollout_rescan_and_auto_refresh(self):
        ...
        handler = (HERE / "webui_handler.py").read_text(encoding="utf-8")
        self.assertIn('"project_tools_protocol": "immediate-registry-v1"', handler)
        ...
        self.assertIn("camera-video paths", source)
        self.assertIn("BATCH_H264_SUMMARY", source)
        manifest_support = (HERE / "static" / "manifest-support.js").read_text(encoding="utf-8")
        self.assertIn("var playbackPath = selectedCameraPath(record);", manifest_support)
