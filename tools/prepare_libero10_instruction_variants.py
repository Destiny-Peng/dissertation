...
records keep the original camera-video mapping and provenance, but receive distinct
...
def select_source_rows(
    ...
    for row in selected:
        ...
        if not isinstance(camera_paths, dict) or not camera_paths:
            raise VariantError(f"Source row has no camera_video_paths: {rollout_id}")
        for camera, video_value in camera_paths.items():
            if not isinstance(video_value, str) or not video_value:
                raise VariantError(
                    f"Source row has an invalid path for {camera!r}: {rollout_id}"
                )
    ...
    *,
    ...
    for row in generated_rows:
        ...
        if row.get("camera_video_paths") != source.get("camera_video_paths"):
            raise VariantError(f"Camera video paths changed for source rollout: {source_id}")
        if row.get("source_camera_video_paths") != source.get("camera_video_paths"):
            raise VariantError(f"Source camera links are not preserved: {source_id}")
