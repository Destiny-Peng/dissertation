"""Rank existing fused-hop sweep configs by interval localization only."""
...
def _add_interval_ranks(rows: list[dict[str, Any]]) -> None:
    ...
    for population_rows in by_population.values():
        max_coverage = max(
            (float(row.get("trigger_coverage") or 0.0) for row in population_rows),
            default=0.0,
        )
        coverage_floor = (
            1.0
    ...
    *,
    ...
    return sorted(
        ...
        key=lambda row: (
            ...
            int(row["rank_mae"]) if row.get("rank_mae") is not None else math.inf,
            str(row["config_id"]),
        ),
    )
